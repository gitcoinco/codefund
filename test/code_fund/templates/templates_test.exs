defmodule CodeFund.TemplatesTest do
  ...
  describe "templates" do
    test "slug_for_property_id/2 requested slug when property_id and requested slug are nil" do
      assert Templates.slug_for_property_id(nil, nil) == "default"
    end

    test "slug_for_property_id/2 requested slug when property_id is nil" do
      assert Templates.slug_for_property_id(nil, "example") == "example"
    end

    test "slug_for_property_id/2 requested slug when property template override does not exist" do
      property = insert(:property)
      assert Templates.slug_for_property_id(property.id, "example") == "example"
    end

    test "slug_for_property_id/2 requested slug when property when template override exists" do
