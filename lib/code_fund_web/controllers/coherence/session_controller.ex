defmodule CodeFundWeb.Coherence.SessionController do
  @moduledoc """
  ...
  * Lockable is enabled and the user is not locked.
    ...
    if valid_user_login?(user, params) do
      if confirmed_access?(user) do
        do_lockable(
          conn,
          login_field,
          [user, user_schema, remember, lockable?, remember, params],
  ...
  defp do_valid_login(nil, _conn, _parms, _opts), do: {:error, :not_found}
  ...
  defp do_valid_login(user, conn, params, opts) do
    ...
    if Config.async_rememberable?() and
         Enum.any?(conn.req_headers, fn {k, v} ->
           k == "x-requested-with" and v == "XMLHttpRequest"
         end) do
